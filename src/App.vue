...
const gerarTecnicos = () => {
    ...
    if (alvo > atual) {
        for (let i = atual + 1; i <= alvo; i++) {
            listaTecnicos.value.push({ id: i, nome: `Técnico ${i}`, restricao: false, pacientes: [], carga: 0, qtdDietas: 0, qtdAcamados: 0 });
        }
    } else if (alvo < atual) {
  ...
  <div class="flex h-screen bg-[#FDFBFD] dark:bg-gray-900 font-sans text-gray-600 dark:text-gray-300 overflow-hidden transition-colors duration-300">
    ...
    <aside class="fixed md:relative inset-y-0 left-0 w-64 bg-white dark:bg-gray-800 border-r border-gray-100 dark:border-gray-700 flex flex-col shadow-2xl md:shadow-sm z-40 transition-transform duration-300 ease-in-out transform"
        ...
        <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
            <div class="nav-item" :class="{ 'active': etapa === 1 }" @click="etapa = 1; isSidebarOpen = false">
                <i class="fa-solid fa-sliders w-6"></i> Configuração
            </div>
            <div class="nav-item" :class="{ 'active': etapa === 2 }" @click="listaLeitos.length > 0 ? (etapa = 2, isSidebarOpen = false) : avanarParaPacientes()">
                <i class="fa-solid fa-bed-pulse w-6"></i> Triagem
            </div>
            <div class="nav-item" :class="{ 'active': etapa === 3 }" @click="resultadoDistribuicao.length > 0 ? (etapa = 3, isSidebarOpen = false) : null">
                <i class="fa-solid fa-clipboard-list w-6"></i> Relatório
            </div>
            
            <div class="mt-8 pt-4 border-t border-gray-50 dark:border-gray-700">
                <div class="nav-item" :class="{ 'active': etapa === 4 }" @click="etapa = 4; isSidebarOpen = false">
        ...
        <main class="flex-1 overflow-y-auto p-4 md:p-10 pb-24 md:pb-10 scroll-smooth">
            <div class="max-w-5xl mx-auto min-h-full flex flex-col">
                    ...
                    <div class="space-y-6">
                        <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
                            ...
                            <div class="space-y-3 mb-6">
                                <div class="p-4 rounded-xl border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/30 flex items-center justify-between cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition" @click="evitarQuebraEnfermaria = !evitarQuebraEnfermaria">
                                    <div>
                                ...
                                <div class="p-4 rounded-xl border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/30 flex items-center justify-between cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition" @click="manterProximidade = !manterProximidade">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-full bg-purple-50 dark:bg-purple-900/20 text-primary flex items-center justify-center">
                                            <i class="fa-solid fa-route"></i>
                                        </div>
                                        <div>
                                            <p class="font-bold text-gray-700 dark:text-white text-sm">Priorizar Vizinhança</p>
                                            <p class="text-[10px] text-gray-400">Tenta agrupar leitos próximos (ex: 201 e 202).</p>
                                        </div>
                                    </div>
                                    <div class="w-12 h-6 rounded-full p-1 transition duration-300 ease-in-out" :class="manterProximidade ? 'bg-primary' : 'bg-gray-300 dark:bg-gray-600'">
                                        <div class="w-4 h-4 bg-white rounded-full shadow-md transform duration-300 ease-in-out" :class="manterProximidade ? 'translate-x-6' : 'translate-x-0'"></div>
                                    </div>
                                </div>
                            </div>
                ...
                <div v-if="etapa === 2">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        <div v-for="item in listaLeitos" :key="item.leito" 
                            class="bg-white dark:bg-gray-800 rounded-xl border transition-all duration-300 relative group overflow-hidden"
                            :class="item.ocupado ? 'border-primary shadow-lg shadow-primary/10 ring-1 ring-primary' : 'border-gray-100 dark:border-gray-700 hover:border-primary/30 hover:shadow-md'"
                        >
                            ...
                            <div v-if="item.ocupado" class="p-4 space-y-4 animate-fade-in">
                                ...
                                <div class="flex gap-2">
                                    <label class="flex-1 cursor-pointer">
                                        ...
                                        <div class="h-16 rounded-lg border border-gray-100 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50 flex flex-col items-center justify-center text-gray-400 peer-checked:bg-green-50 peer-checked:dark:bg-green-900/30 peer-checked:border-green-400 peer-checked:text-green-600 transition hover:bg-white dark:hover:bg-gray-700 hover:shadow-sm"><i class="fa-solid fa-person-walking text-lg mb-1"></i><span class="text-[9px] font-bold uppercase">Leve</span></div>
                                    </label>
                                    <label class="flex-1 cursor-pointer">
                                        <input type="radio" value="semi" v-model="item.mobilidade" class="hidden peer">
                                        <div class="h-16 rounded-lg border border-gray-100 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50 flex flex-col items-center justify-center text-gray-400 peer-checked:bg-yellow-50 peer-checked:dark:bg-yellow-900/30 peer-checked:border-yellow-400 peer-checked:text-yellow-600 transition hover:bg-white dark:hover:bg-gray-700 hover:shadow-sm"><i class="fa-solid fa-wheelchair text-lg mb-1"></i><span class="text-[9px] font-bold uppercase">Médio</span></div>
                                    </label>
                                    <label class="flex-1 cursor-pointer">
                                        <input type="radio" value="acamado" v-model="item.mobilidade" class="hidden peer">
                                        <div class="h-16 rounded-lg border border-gray-100 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50 flex flex-col items-center justify-center text-gray-400 peer-checked:bg-red-50 peer-checked:dark:bg-red-900/30 peer-checked:border-red-400 peer-checked:text-red-600 transition hover:bg-white dark:hover:bg-gray-700 hover:shadow-sm"><i class="fa-solid fa-bed-pulse text-lg mb-1"></i><span class="text-[9px] font-bold uppercase">Pesado</span></div>
                                    </label>
                                </div>
                                ...
                                <div v-if="item.mobilidade === 'acamado'" class="bg-red-50 dark:bg-red-900/20 p-3 rounded-lg border border-red-100 dark:border-red-800 animate-fade-in">
                                    ...
                                    <div class="flex gap-2">
                                        <label class="flex-1 cursor-pointer">
                                            ...
                                            <div class="py-2 text-center text-xs font-bold rounded border border-red-200 dark:border-red-700 text-red-400 dark:text-red-300 bg-white dark:bg-gray-800 peer-checked:bg-red-200 peer-checked:dark:bg-red-800 peer-checked:text-red-700 peer-checked:dark:text-white transition">Boa</div>
                                        </label>
                                        <label class="flex-1 cursor-pointer">
                                            <input type="radio" value="pouca" v-model="item.grauAcamado" class="hidden peer">
                                            <div class="py-2 text-center text-xs font-bold rounded border border-red-200 dark:border-red-700 text-red-400 dark:text-red-300 bg-white dark:bg-gray-800 peer-checked:bg-red-500 peer-checked:text-white transition">Pouca</div>
                                        </label>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-2 pt-2 border-t border-gray-50 dark:border-gray-700">
                                    <label class="flex items-center gap-2 cursor-pointer group/chk">
                                        ...
                                        <span class="text-xs text-gray-600 dark:text-gray-300 group-hover/chk:text-purple-600 transition">Usa SNE / GTM</span>
                                    </label>
                                    ...
                                    <label class="flex items-center gap-2 cursor-pointer group/chk">
                                        ...
                                        <span class="text-xs text-gray-600 dark:text-gray-300 font-bold group-hover/chk:text-blue-500 transition">Irrigação (+2)</span>
                                    </label>
                                    ...
                                    <label class="flex items-center gap-2 cursor-pointer group/chk">
                                        ...
                                        <span class="text-xs font-bold text-gray-600 dark:text-gray-300 group-hover/chk:text-red-600 transition flex items-center gap-1"><i class="fa-solid fa-triangle-exclamation text-[10px]"></i> Isolamento</span>
                                    </label>
                                </div>
                                ...
                                <div class="bg-gray-50 dark:bg-gray-700/50 border border-gray-100 dark:border-gray-700 rounded-lg p-3 space-y-2">
                                    ...
                                    <div class="flex gap-2">
                                        <button v-for="n in 3" :key="n" @click="item.grauExtra = item.grauExtra === n ? 0 : n"
                                            ...
                                            :class="item.grauExtra === n ? 'bg-gray-800 dark:bg-gray-600 text-white border-gray-800 dark:border-gray-600' : 'bg-white dark:bg-gray-800 text-gray-400 border-gray-200 dark:border-gray-600 hover:border-gray-400'">
                                            +{{ n }}
                                        </button>
                                    </div>
                                    <input v-if="item.grauExtra > 0" type="text" v-model="item.motivoExtra" 
                                        class="w-full text-xs p-2 bg-white dark:bg-gray-800 dark:text-white rounded border border-gray-200 dark:border-gray-600 focus:border-gray-400 outline-none" 
                        ...
                        <div v-for="tec in resultadoDistribuicao" :key="tec.id" class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden flex flex-col h-full">
                            <div class="p-5 border-b border-gray-50 dark:border-gray-700 flex justify-between items-start bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-700">
                                <div class="flex gap-4">
                                    ...
                                    <div>
                                        <h3 class="font-bold text-lg text-gray-800 dark:text-white">{{ tec.nome }}</h3>
                                        <div class="flex gap-2 mt-1">
                                            <span class="text-[10px] font-bold bg-primary-light dark:bg-gray-600 text-primary dark:text-white px-2 py-1 rounded">Carga: {{ tec.carga }}</span>
                            ...
                            <div class="p-4 flex-1 bg-white dark:bg-gray-800 space-y-2">
                                <div v-for="p in tec.pacientes" :key="p.leito" class="p-3 rounded-xl border flex flex-col gap-2 group hover:shadow-md transition" :class="p.isolamento ? 'border-l-4 border-l-red-500 bg-red-50/10 dark:bg-red-900/20 border-gray-100 dark:border-gray-700' : 'border-l-4 border-l-primary bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700'">
                                    ...
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center gap-2">
                                            <span class="bg-gray-800 dark:bg-gray-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded">{{ p.leito }}</span>
                                            <span class="font-bold text-gray-700 dark:text-gray-200 text-sm">{{ p.nome || 'Paciente' }}</span>
                                        </div>
                                        <div class="text-lg font-bold text-gray-200 dark:text-gray-600 group-hover:text-primary-light transition">{{ p.peso }}</div>
                                    </div>
                                    ...
                                    <div v-if="p.grauExtra > 0 && p.motivoExtra" class="text-[10px] text-gray-500 dark:text-gray-400 italic border-t border-gray-100 dark:border-gray-700 pt-1 mt-1">
                                        Obs: {{ p.motivoExtra }}
                                    </div>
                                </div>
                    ...
                    <div class="space-y-6">
                        <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                <div class="p-4 rounded-xl border border-green-100 dark:border-green-800 bg-green-50 dark:bg-green-900/20">
                                    <div class="text-2xl font-bold text-green-600 dark:text-green-400 mb-1">1 Ponto</div>
                                    <div class="font-bold text-green-800 dark:text-green-300 text-sm">Leve (Deambula)</div>
                                    <p class="text-xs text-green-600 dark:text-green-400 mt-1">Independente para banho e locomoção.</p>
                                </div>
                                <div class="p-4 rounded-xl border border-yellow-100 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-900/20">
                                    <div class="text-2xl font-bold text-yellow-600 dark:text-yellow-400 mb-1">2 Pontos</div>
                                    <div class="font-bold text-yellow-800 dark:text-yellow-300 text-sm">Médio (Semi)</div>
                                    <p class="text-xs text-yellow-600 dark:text-yellow-400 mt-1">Precisa de auxílio parcial ou banho de aspersão com cadeira.</p>
                                </div>
                                <div class="p-4 rounded-xl border border-red-100 dark:border-red-800 bg-white dark:bg-gray-800">
                                    <div class="text-2xl font-bold text-red-400 dark:text-red-300 mb-1">3 Pontos</div>
                                    <div class="font-bold text-red-500 dark:text-red-300 text-sm">Pesado (Boa Mobilidade)</div>
                                    <p class="text-xs text-red-400 dark:text-red-300 mt-1">Acamado, mas ajuda na mudança de decúbito.</p>
                                </div>
                                <div class="p-4 rounded-xl border border-red-100 dark:border-red-800 bg-red-50 dark:bg-red-900/20">
                                    <div class="text-2xl font-bold text-red-600 dark:text-red-400 mb-1">4 Pontos</div>
                                    <div class="font-bold text-red-800 dark:text-red-300 text-sm">Pesado (Pouca Mobilidade)</div>
                                    <p class="text-xs text-red-600 dark:text-red-400 mt-1">Totalmente dependente. Exige esforço físico máximo.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div class="p-4 rounded-xl border border-purple-100 dark:border-purple-800 bg-purple-50 dark:bg-purple-900/20 flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-white dark:bg-gray-700 text-purple-600 dark:text-purple-300 flex items-center justify-center font-bold text-lg shadow-sm">+1</div>
                                    <div>
                                        <div class="font-bold text-purple-800 dark:text-purple-300 text-sm">SNE / GTM</div>
                                        <p class="text-xs text-purple-600 dark:text-purple-400">Dieta enteral ou Gastrostomia.</p>
                                    </div>
                                </div>
                                <div class="p-4 rounded-xl border border-blue-100 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/20 flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-white dark:bg-gray-700 text-blue-600 dark:text-blue-300 flex items-center justify-center font-bold text-lg shadow-sm">+2</div>
                                    <div>
                                        <div class="font-bold text-blue-800 dark:text-blue-300 text-sm">Irrigação</div>
                                        <p class="text-xs text-blue-600 dark:text-blue-400">Vesical contínua. Troca frequente de soro.</p>
                                    </div>
                                </div>
                                <div class="p-4 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-white dark:bg-gray-600 text-gray-800 dark:text-white flex items-center justify-center font-bold text-lg shadow-sm">1-3</div>
                                    <div>
                                        <div class="font-bold text-gray-800 dark:text-white text-sm">Grau Extra</div>
                                        <p class="text-xs text-gray-500 dark:text-gray-400">Complexidade familiar ou comportamental.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
                            ...
                            <ul class="space-y-4 text-sm text-gray-600 dark:text-gray-300">
                                <li class="flex gap-3">
                                    <div class="min-w-[24px] h-6 rounded-full bg-red-100 dark:bg-red-900/20 text-red-600 dark:text-red-400 flex items-center justify-center text-xs font-bold">1</div>
                                ...
                                <li class="flex gap-3">
                                    <div class="min-w-[24px] h-6 rounded-full bg-red-100 dark:bg-red-900/20 text-red-600 dark:text-red-400 flex items-center justify-center text-xs font-bold">2</div>
                                ...
                                <li class="flex gap-3">
                                    <div class="min-w-[24px] h-6 rounded-full bg-primary-light dark:bg-gray-700 text-primary dark:text-white flex items-center justify-center text-xs font-bold">3</div>
                                ...
                                <li class="flex gap-3">
                                    <div class="min-w-[24px] h-6 rounded-full bg-purple-100 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 flex items-center justify-center text-xs font-bold">4</div>
                                    <p><strong>Vizinhança:</strong> Tenta manter leitos próximos com o mesmo técnico (ex: 201 e 202) para reduzir deslocamento.</p>
                                </li>
                                <li class="flex gap-3">
                                    <div class="min-w-[24px] h-6 rounded-full bg-gray-800 dark:bg-gray-600 text-white flex items-center justify-center text-xs font-bold">5</div>
                ...
                <div class="mt-12 py-6 border-t border-gray-100 dark:border-gray-700 flex flex-col md:flex-row items-center justify-between gap-4">
                    ...
                    <div class="flex gap-4 w-full md:w-auto">
                        <button v-if="etapa > 1 && etapa !== 4" @click="etapa--" 
                            class="flex-1 md:flex-none px-6 py-3 rounded-xl font-bold text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-primary transition">
                            Voltar
                        </button>
                        ...
                        <button v-if="etapa === 2" @click="calcular" 
                         ...
                         <button v-if="etapa === 3" @click="reiniciar" 
                            class="flex-1 md:flex-none px-8 py-3 rounded-xl bg-gray-800 dark:bg-gray-600 text-white font-bold shadow-lg hover:bg-gray-900 dark:hover:bg-gray-500 transition transform active:scale-95">
                            Novo Plantão
                        </button>
                    </div>
                </div>
                ...
                <footer class="mt-8 pt-10 border-t border-gray-100 dark:border-gray-700 text-center pb-10 bg-white dark:bg-gray-800 rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.02)]">
                    <p class="text-gray-600 dark:text-gray-300 mb-6 font-medium text-lg tracking-tight">
